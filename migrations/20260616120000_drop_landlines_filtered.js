exports.up = function up(knex) {
  return knex.schema.raw(`
    drop table filtered_contact;

    drop view campaign cascade;

    alter table all_campaign drop column landlines_filtered;
...
exports.down = function down(knex) {
  return knex.schema.raw(`
    create table public.filtered_contact (
      ...
      campaign_id integer not null references public.all_campaign(id),
      ...
      filtered_reason text not null check (filtered_reason = any (array['INVALID'::text, 'LANDLINE'::text, 'VOIP'::text, 'OPTEDOUT'::text])),
      constraint filtered_contact_cell_campaign_id_unique unique (cell, campaign_id)
    );
    ...
    alter table all_campaign add column landlines_filtered boolean default false;
